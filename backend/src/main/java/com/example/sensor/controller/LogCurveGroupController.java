...
public class LogCurveGroupController {
    ...
    public ResponseEntity<Map<String, Object>> getCurvesByGroupId(
        ...
        try {
            ...
            logCurveGroupService.enrichCurveNames(curves);
    ...
    public ResponseEntity<Map<String, Object>> getAllGrouped(
        ...
        try {
            ...
            // 填充曲线名称
            for (List<LogCurveGroup> curves : grouped.values()) {
                logCurveGroupService.enrichCurveNames(curves);
            }
    ...
    /**
     * 获取所有数据，按月份+日期+组名称ID三级分组返回
     * GET /api/curve-group/all-grouped-by-date?month=2025-12
     */
    @GetMapping("/all-grouped-by-date")
    public ResponseEntity<Map<String, Object>> getAllGroupedByDate(
            @RequestParam(required = false) String month) {
        try {
            Map<String, Map<String, Map<Integer, List<LogCurveGroup>>>> grouped;
            if (month != null && !month.isEmpty()) {
                grouped = logCurveGroupService.getAllGroupedByDate(month);
            } else {
                grouped = new LinkedHashMap<>();
            }
            // 填充曲线名称
            for (Map<String, Map<Integer, List<LogCurveGroup>>> monthData : grouped.values()) {
                for (Map<Integer, List<LogCurveGroup>> dateData : monthData.values()) {
                    for (List<LogCurveGroup> curves : dateData.values()) {
                        logCurveGroupService.enrichCurveNames(curves);
                    }
                }
            }
            ...
            logger.error("获取三级分组曲线数据失败", e);
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("code", 500);
            errorResponse.put("message", "获取三级分组曲线数据失败: " + e.getMessage());
            return ResponseEntity.status(500).body(errorResponse);
        }
    }
}
